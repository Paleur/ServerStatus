...
client_log_file="/var/log/serverstatus_client.log"
server_log_file="/var/log/serverstatus_server.log"
jq_file="${file}/jq"
...
Download_Server_Status_server(){
	cd "/tmp"
	wget -N --no-check-certificate "https://github.com/Paleur/ServerStatus/archive/master.zip"
...
Download_Server_Status_client(){
	cd "/tmp"
	wget -N --no-check-certificate "https://raw.githubusercontent.com/Paleur/ServerStatus/master/clients/status-client.py"
...
Service_Server_Status_server(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/Paleur/ServerStatus/master/service/server_status_server_centos" -O /etc/init.d/status-server; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/Paleur/ServerStatus/master/service/server_status_server_debian" -O /etc/init.d/status-server; then
			echo -e "${Error} ServerStatus 服务端服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/status-server
...
Service_Server_Status_client(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/Paleur/ServerStatus/master/service/server_status_client_centos" -O /etc/init.d/status-client; then
		...
		if ! wget --no-check-certificate "https://raw.githubusercontent.com/Paleur/ServerStatus/master/service/server_status_client_debian" -O /etc/init.d/status-client; then
			echo -e "${Error} ServerStatus 客户端服务管理脚本下载失败 !" && exit 1
		fi
		chmod +x /etc/init.d/status-client
	...
	if [[ ${mode} == "server" ]]; then
		...
		if [[ ${release} == "centos" ]]; then
			...
			if [[ -z ${python_status} ]]; then
				yum install -y python unzip make
				yum groupinstall "Development Tools" -y
			else
				yum install -y unzip make
				yum groupinstall "Development Tools" -y
			fi
		else
			apt-get update
			if [[ -z ${python_status} ]]; then
				apt-get install -y python unzip build-essential make
			else
				apt-get install -y unzip build-essential make
			fi
		fi
	else
		python_status=$(python --help)
...
Set_server(){
	...
	[[ -z ${mode} ]] && mode="server"
	if [[ ${mode} == "server" ]]; then
		echo -e "请输入 ServerStatus 服务端显示网站要设置的 域名[server]，如果要使用本机IP，请留空直接回车"
...
Set_server_port(){
	while true
		...
		read -e -p "(默认: 1901):" server_port_s
		[[ -z "$server_port_s" ]] && server_port_s="1901"
...
Set_password(){
	...
	read -e -p "(默认: Paleur):" password_s
	[[ -z "$password_s" ]] && password_s="Paleur"
	echo && echo "	================================================"
...
Install_caddy(){
	...
	if [[ "${caddy_yn}" == [Yy] ]]; then
		...
		if [[ ! -e "/usr/local/caddy/caddy" ]]; then
			wget -N --no-check-certificate https://raw.githubusercontent.com/Paleur/World/master/caddy_install.sh
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/Paleur/ServerStatus/master/status.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	...
	wget -N --no-check-certificate "https://raw.githubusercontent.com/Paleur/ServerStatus/master/status.sh" && chmod +x status.sh
